...
#def write_json(new_data,dict,filename='dnd_save_load.save_data'):
#def write_json(new_data,filename='dnd_save_load.save_data'):
#def write_json(new_data):

#Writes information to the json file. The different "dict" keys are for nested dictionaries. If dict2 is "Equipment", use equip_mod, and if dict2 is "Misc", use misc_mod
#def write_json(dict1,dict2,dict3,new_data):
#    if dict1== 'Equipment':
#        dnd_skills.equip_mod(dict2,new_data)
#
#    if dict2 =='null':
#        with open(dnd_save_load.save_data,'r+') as file:
#            file_data = json.load(file)
#            file_data[dict1] = new_data
#            file.seek(0)
...
#        with open(dnd_save_load.save_data,'r+') as file:
#            file_data = json.load(file)
#            file_data[dict1][dict2] = new_data
#            file.seek(0)
#            json.dump(file_data,file,indent=4)
#
#    else:
#        with open(dnd_save_load.save_data,'r+') as file:
#            file_data = json.load(file)
#            file_data[dict1][dict2][dict3] = new_data
#            file.seek(0)
#            json.dump(file_data,file,indent=4)
...
#    with open(dnd_save_load.save_data,'r+') as file:
#        file_data = json.load(file)
##        file_data[dict].append(new_data)
#        file_data[dict] = new_data
#        file.seek(0)
#        json.dump(file_data,file,indent=4)



# Enter the character's name with a maximum of 15 characters and no special characters
def name_selection():
    while True:
        name_input = input("Enter your name (max 15 characters): ")
        regex = re.compile(r'^[a-zA-Z0-9]*$')

        if len(name_input) <=15 and bool(regex.search(name_input)) == True:
            dnd_dict.character_dict["name"] = name_input
            break

        elif regex.search(name_input) == None:
            print("Do not use special characters. Please select again")
            continue
...
def stat_selection():
#    dnd_dict.character_dict.clear()
#    dnd_dict.character_dict.update(backup_dict)
    name_selection()
    while True:
